<template><div class="box-page-container flex flex-column">
  <div class="grid justify-content-between">
    <div class="col-fixed">
      <h1 class="font-bold m-0 font-size-4xlarge line-height-1">Danh sách người dùng</h1><span class="text-600 font-size-small" v-if="boxData">{{ boxData.length }} products found</span>
    </div>
    <div class="col-fixed">
      <div class="grid">
  ...
  <div class="grid grid-nogutter flex-1 relative overflow-hidden">
    <div class="col h-full absolute top-0 left-0 right-0">
      <DataTable class="w-full airtag-datatable h-full flex flex-column" v-if="boxData" :value="boxData" responsiveLayout="scroll" :selection.sync="selectedBoxes" dataKey="id" :resizableColumns="true" :rows="20" :scrollable="false">
        ...
        <Column field="code" header="SỐ ĐIỆN THOẠI" sortable="sortable" bodyClass="font-semibold"></Column>
        ...
        <Column field="attribute" header="NGÀY CẬP NHẬT" sortable="sortable" className="p-text-right" bodyClass="font-semibold">
          <template #body="{ data }">{{ new Date(data.createAt).toLocaleDateString('en-US') }}</template>
        </Column>
        <Column field="status" header="TRẠNG THÁI" sortable="sortable" className="p-text-right">
          <template #body="{ data }">
            <div>
              <Tag class="px-2 bg-green-100" v-if="data.status" severity="success"><span class="font-bold text-green-400 font-size-small">AVAILABLE</span></Tag>
              <Tag class="px-2 surface-200" v-else="v-else" severity="success"><span class="font-bold text-400 font-size-small">DISABLE</span></Tag>
            </div>
          </template>
        </Column>
